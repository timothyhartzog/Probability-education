...
// Complete chapter registry for Probability: Foundations to Research Frontiers

export type Difficulty = 'high-school' | 'undergrad' | 'graduate' | 'research';
...
export interface Chapter {
  ...
  minutes: number;
  topics: string[];
  legacyModule?: string;   // existing src/modules/ path to iframe
  hasNewSims: boolean;     // has new Svelte-native simulations
  route: string;
}
...
export function partColor(num: number): string {
  return PARTS[num - 1]?.color ?? 'var(--accent)';
}
export function getChapter(partNum: number, chapterNum: number): Chapter | undefined {
  return CHAPTERS.find(c => c.part === partNum && c.num === chapterNum);
}
export function getNext(ch: Chapter): Chapter | undefined {
  const idx = CHAPTERS.indexOf(ch);
  return CHAPTERS[idx + 1];
}
export function getPrev(ch: Chapter): Chapter | undefined {
  const idx = CHAPTERS.indexOf(ch);
  return CHAPTERS[idx - 1];
}
